...
class MyApp(QMainWindow):
    ...
    def update_domain_list(self):
        ...
        cf = CloudFlare(token=self.api_key)
        zones = cf.zones.get()
        total_zones = len(zones)  # Get total number of zones
        for i, zone in enumerate(zones):  # Use enumerate to get the current index
            zone_id = zone['id']
            dns_records = cf.zones.dns_records.get(zone_id)
            for dns_record in dns_records:
                if dns_record['type'] == 'A':
            ...
            # Update progress bar
            progress = int((i + 1) / total_zones * 100)  # Calculate progress as a percentage
